...
- Existing rows that use legacy values (`owner` | `member`) must be mapped to API roles (`admin` | `user`) in service-layer adapters.
...
- `project_members` insert for creator (`role=admin`).
- `project_members` inserts for teammates (`role=user`).

Validation:

- Enforce UNIQUE (`project_id`, `user_id`) to prevent duplicate members.
- Only admin/user project members can query project internals.
...
- `tasks.assignee_user_id` changes only through accepted reassignment requests (or admin override endpoint if added).
