...
## 2026-02-21T21:43:12Z

- decision: Final QA signoff is accepted with explicit scaffold-mode caveats while upstream API tickets remain open, instead of blocking all role-path verification.
- rationale: Core role-aware navigation and behavior paths are testable today, and known API gaps are isolated with clear UI error/scaffold messaging.
- alternatives considered: postpone `PB-029` until all dependent API issues merge; rejected because it delays integration visibility and handoff quality.
- impact: Published `docs/qa/post-onboarding-role-matrix.md` with role matrix, evidence, and regression follow-ups; added final handoff summary and actionable gap list.
